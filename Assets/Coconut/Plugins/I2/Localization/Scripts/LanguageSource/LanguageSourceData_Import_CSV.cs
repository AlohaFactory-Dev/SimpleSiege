...
namespace I2.Loc
{
	public partial class LanguageSourceData
	{
		...
		public string Import_CSV( string Category, List<string[]> CSV, eSpreadsheetUpdateMode UpdateMode = eSpreadsheetUpdateMode.Replace )
		{
			...
			var ValidColumnName_Ver = new[]{ "Version" };
			...
			if (Tokens.Length>1 && ArrayContains(Tokens[0], ValidColumnName_Key))
			{
				...
				if (Tokens.Length>2)
				{
                    ...
                    if (ArrayContains(Tokens[1], ValidColumnName_Ver))
                    {
                        LanguagesStartIdx = 2;
                    }
				}
				if (Tokens.Length>3)
				{
                    ...
                    if (ArrayContains(Tokens[2], ValidColumnName_Ver))
                    {
                        LanguagesStartIdx = 3;
                    }
				}
                if (Tokens.Length>4)
                {
                    if (ArrayContains(Tokens[3], ValidColumnName_Type)) 
                    {
                        TypeColumnIdx = 3;
                        LanguagesStartIdx = 4;
                    }
                    if (ArrayContains(Tokens[3], ValidColumnName_Desc)) 
                    {
                        DescColumnIdx = 3;
                        LanguagesStartIdx = 4;
                    }
                    if (ArrayContains(Tokens[3], ValidColumnName_Ver))
                    {
                        LanguagesStartIdx = 4;
                    }
                }
